...
- **Severity indicator support** with visual red indicator
...
The Checkbox component supports an optional severity indicator that displays a red square to highlight important or critical items. This feature is useful for marking high-priority items, errors, or items requiring special attention.

**Default value**: `severity={false}` (severity indicator is off by default)

<Canvas of={CheckboxStories.WithSeverity} />

### Severity with RTL Layout

The severity indicator properly adapts to right-to-left layouts, maintaining correct positioning relative to the checkbox icon.

<Canvas of={CheckboxStories.WithSeverityRTL} />
...
### Severity Indicator in RTL
<Canvas of={CheckboxStories.WithSeverityRTL} />

### Severity Comparison
<Canvas of={CheckboxStories.SeverityComparison} />
...
- **Priority marking**: Use severity indicator for high-priority, critical, or error items
- **Status indication**: Mark items requiring special attention or immediate action
...
- **Severity levels**: The severity indicator is boolean (on/off) - use dedicated severity components for multiple levels
